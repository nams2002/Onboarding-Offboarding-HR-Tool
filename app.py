...
def convert_html_to_pdf(html_content):
    """Convert HTML content to PDF bytes using ReportLab"""
    try:
        # For Streamlit Cloud compatibility, we'll create a simple text-based PDF
        # This is a fallback that works without complex dependencies
        buffer = io.BytesIO()
        ...
        # Add a simple message about PDF generation
        ...
        story.append(Paragraph("Document Generated Successfully", title_style))
        ...
        story.append(Paragraph("This document was generated by the Rapid Innovation Onboarding System.", content_style))
        story.append(Paragraph("For the full formatted version, please use the preview feature in the application.", content_style))
        story.append(Spacer(1, 20))
        story.append(Paragraph("Thank you for using our onboarding automation system!", content_style))
...
if page == "🏠 Home":
    ...
    if st.session_state.offer_letter_html and st.session_state.offer_letter_data:
        ...
        # Notice for Streamlit Cloud users
        st.info("📝 **Note**: PDF generation is simplified for Streamlit Cloud compatibility. Use the preview above to view the formatted document.")
        ...
        with col2:
            if st.button("📧 Send Email with PDF"):
                ...
                if pdf_bytes:
                    ...
                    # If PDF generation fails, send email with HTML content only
                    enhanced_email_body = email_body + f"""
                    <hr>
                    <h3>Offer Letter Details:</h3>
                    {st.session_state.offer_letter_html}
                    """
